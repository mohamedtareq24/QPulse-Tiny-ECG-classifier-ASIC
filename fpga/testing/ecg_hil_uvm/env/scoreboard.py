...
class ECGScoreboard(uvm_component):
    ...
    async def collect_sent_data(self):
        while True:
            tx_item = await self.dut_rx_fifo.get()
            if hasattr(tx_item, "samples_10b"):
                self.sent_sample_count += len(getattr(tx_item, "samples_10b") or [])
    ...
    async def compare_received(self):
        ...
        while True:
            try:
                rx_item = await asyncio.wait_for(
                    self.dut_tx_fifo.get(),
                    timeout=self.cfg.response_timeout_s,
                )
            except asyncio.TimeoutError:
                self.logger.warning(
                    "Timed out waiting for DUT UART response (timeout=%.3fs)",
                    self.cfg.response_timeout_s,
                )
                continue
